...
int8_t write_page_to_hd(uint32_t seitennummer, uint32_t virt_address) { // alte addresse! nicht die neue!
    ...
    if (seitentabelle[seitennummer].dirty_bit) {
        // Simulate writing the page back to the hard disk
        // In a real system, you would write the page to the actual hard disk
...
// Extract the page number from the virtual address
    ...
    if (seitentabelle[page_number].dirty_bit) {
        // Write the page back to the hard disk
        write_page_to_hd(page_number, virt_address);
    }

    // Perform the actual swapping of pages
    // Swap in the new page from the hard disk to RAM
...
void set_data(uint32_t virt_address, uint8_t value) {
    ...
    if (!seitentabelle[get_seiten_nr(virt_address)].present_bit) {
        // If not present, load the page from the hard disk
        get_page_from_hd(virt_address);
    }

    // Update the data in RAM using the physical address
    ra_mem[physical_address] = value;

    // Mark the page as dirty since it's modified
